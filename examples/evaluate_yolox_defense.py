...
original_images_dir = r"D:/Merged Datasets/images"
...
def run_inference_on_images(model, coco, image_dir, output_json, filename_map=None, use_filename_key=False, defense_model=None):
    detections = []
    ...
    for img_id in tqdm(coco.getImgIds(), desc="Running inference"):
        ...
        # Get image path based on map (if provided)
        ...
        bboxes = output[:, 0:4] / scale * 1.2
        scores = output[:, 4] * output[:, 5]
        cls_ids = output[:, 6].astype(np.int32)

        # Adapted for JSON format with detections
...
if __name__ == "__main__":
    ...
    for orig in df["orig_images"].unique():
        ...
        # Save only the first (or choose logic to cycle through all)
        original_to_attacked[orig] = originals_attacks[0]
    ...
    #2. Attacked Images (use only 1 attack per original for now)
